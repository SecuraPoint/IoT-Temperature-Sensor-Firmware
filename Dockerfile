...
# Generate SBOM as part of the build (syft wont find any dependency since OpenSSL is statically linked)
RUN syft ./build/iot_temp_sensor --output cyclonedx-json > ./build/sbom-syft-build.json

# Generate experimental SBOM with dynamically linked libraries (syft wont find any dependency since OpenSSL is statically linked)
